...
class Command(BaseCommand):
    ...
    def populate_verses_from_json(self, file_path, version):
        ...
        try:
            ...
            # Ler arquivo com encoding correto (utf-8-sig remove BOM automaticamente)
    ...
    def populate_verses_from_api(self, version):
        ...
        try:
            ...
            # Decodificar corretamente, removendo BOM se existir
            ...
            for book_data in data:
                ...
                for chapter_num, chapter_verses in enumerate(chapters, start=1):
                    for verse_num, verse_text in enumerate(chapter_verses, start=1):
                        ...
                        # Progress indicator a cada 1000 versículos
                        if verses_created % 1000 == 0:
                            self.stdout.write(f'   📖 {verses_created} versículos processados...')
